...
const isOfferCartItem = (ci) =>
  ...
  const handleQuantityChange = async (itemId, change) => {
    try {
      ...
      const cartItem = currentCartData?.items?.find(
        item => item.itemId === itemId || item.itemId === itemId.toString()
      );
      if (!cartItem) {
      ...
      // Block qty changes for offer lines
      if (isOfferCartItem(cartItem)) {
        toast.info('Offer items cannot be updated here. Remove and re-add the offer pack.');
        return;
      }
      ...
      if (cartItem.loose) {
        ...
        const result = await updateCartItem(itemId, newQuantity, { totalPrice: calculatedPrice });
        if (!result.success) toast.error(result.error || 'Failed to update quantity');
      } else {
  ...
  // IMPORTANT: Offer items are line-priced; do NOT multiply by quantity
  ...
  const calculateTotal = () => {
    ...
    return cart.items.reduce((total, item) => total + lineTotal(item), 0);
  };
              ...
              <div className="p-6">
                ...
                <div className="space-y-4">
                  {cart.items.map((item, index) => {
                    const offer = isOfferCartItem(item);
                        ...
                        <div className="flex items-center gap-3 sm:hidden">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-gray-600 text-xs mt-1">
                              ...
                              {offer && item.quantityLabel && (
                                <span className="text-[11px] text-yellow-700 ml-1"> • {item.quantityLabel}</span>
                              )}
                            </div>
                            {offer && (
                              <div className="text-[11px] text-yellow-700 mt-1">Offer applied</div>
                            )}
                          </div>
                          <div className={`flex items-center border rounded-full px-2 py-1 bg-white shadow-sm ml-2 ${offer ? 'opacity-50 cursor-not-allowed' : ''}`}>
                            <button
                              onClick={() => !offer && handleQuantityChange(item.itemId, -1)}
                              disabled={offer}
                              className="w-6 h-6 flex items-center justify-center text-lg text-gray-600"
                            >−</button>
                        ...
                        <div className="hidden sm:flex items-center gap-4">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-gray-500">
                              ₹{item.totalPrice} {renderQuantityLabel(item)}
                              {offer && item.quantityLabel && (
                                <span className="ml-1 text-yellow-700"> • {item.quantityLabel}</span>
                              )}
                            </div>
                            {offer && (
                              <div className="text-xs text-yellow-700 mt-1">Offer applied</div>
                            )}
                          </div>

                          <div className={`flex items-center gap-3 ${offer ? 'opacity-50 cursor-not-allowed' : ''}`}>
                            <button
                              onClick={() => !offer && handleQuantityChange(item.itemId, -1)}
                              disabled={offer}

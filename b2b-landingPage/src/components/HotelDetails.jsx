                    ...
                    <div className="w-full">
                        ...
                        <div className="mt-8">
                            ...
                            {menu && menu.length > 0 ? (
                                <div className="space-y-4">
                                    {menu.map(category => {
                                        // Case 1: category is 'uncategorized' and only subcategory is 'general'
                                        if (
                                            category.category === 'uncategorized' &&
                                            category.subcategories.length === 1 &&
                                            category.subcategories[0].subcategory === 'general'
                                        ) {
                                            // Show items directly, no accordion
                                            return (
                                                <div key={category.category} className="bg-white rounded-lg shadow-lg overflow-hidden p-4">
                                                    <div className="space-y-4">
                                        ...
                                        // Case 2: category is not 'uncategorized' and all subcategories are 'general'
                                        if (
                                            category.category !== 'uncategorized' &&
                                            category.subcategories.length === 1 &&
                                            category.subcategories[0].subcategory === 'general'
                                        ) {
                                            // Show category accordion, inside show items directly
                                            return (
                                        ...
                                        // Case 3: category and subcategory are both custom
                                        return (
                                            <div key={category.category} className="bg-white rounded-lg shadow-lg overflow-hidden">
                                                <button
                                                    onClick={() => toggleCategory(category.category)}
                                                    ...
                                                    <div className="p-4 border-t">
                                                        {category.subcategories.map(subcategory => (
                                                            subcategory.subcategory === 'general' ? (
                                                                <div key={subcategory.subcategory} className="space-y-4">
                                                                ...
                                                                <div key={subcategory.subcategory} className="mb-6">
                                                                    <button
                                                                        onClick={() => toggleSubcategory(category.category, subcategory.subcategory)}
                                                                        className="w-full flex justify-between items-center py-2 px-3 bg-gray-50 rounded hover:bg-gray-100 mb-2"
                                                                    >
                                                                        <h4 className="text-lg font-medium">{subcategory.subcategory}</h4>
                                                                        <span className="text-xl font-bold">
                                                                            {expandedSubcategories[category.category]?.[subcategory.subcategory] ? '−' : '+'}
                                                                        </span>
                                                                    </button>
                                                                    {expandedSubcategories[category.category]?.[subcategory.subcategory] && (
                                                                        <div className="space-y-4">

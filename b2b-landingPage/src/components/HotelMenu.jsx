    ...
    <div className="max-w-4xl mx-auto bg-white">
      ...
      {showMenuOverlay && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center">
          <div className="bg-black text-white p-8 rounded-lg w-80 max-w-sm">
      ...
      {/* Menu Button */}
      <div className="fixed bottom-6 right-32 z-40">
        <button
      ...
      <div className="p-4">
        {menu.map((cat) => (
          <div key={cat.category} id={`category-${cat.category}`} className="mb-8">
            {/* Category Header */}
            <div className="mb-4">
              <h2 className="text-2xl font-bold text-gray-800 mb-2">{formatName(cat.category)}</h2>
            </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        {/* <div className="flex items-center gap-1 mb-2">
                          ...
                          <span className="text-sm text-gray-600">4.1 (12)</span>
                        </div> */}
                        {item.description && <p className="text-gray-600 text-sm mb-2">{item.description}</p>}
                        {/* <button className="text-blue-600 text-sm hover:underline">more</button> */}
                      </div>
                      <div className="flex flex-col items-end gap-2">
                        ...
                        <button
                          className={`px-6 py-2 rounded-lg font-semibold ${item.inStock
                    ...
                    {expandedSubcategories.has(`${cat.category}--${subcat.subcategory}`) && (
                      <div className="border-t border-gray-200">
                        {subcat.items.map((item) => (
                          <div key={item._id} className="p-4 border-b border-gray-100 last:border-b-0">
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-2">
                                  {item.foodType === "veg" ? <VegIcon /> : <NonVegIcon />}

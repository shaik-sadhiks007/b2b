...
const EditMenu = () => {
    ...
    const fetchMenuData = async () => {
        ...
        setFormData({
            ...
            // Separate templates and custom items
            const templateItems = data.morning.filter(item => 
                templates.some(t => t.menuName === item.menuName && t.price === item.price)
            );
            const customItems = data.morning.filter(item => 
                !templates.some(t => t.menuName === item.menuName && t.price === item.price)
            );
            
            setSelectedTemplates(templateItems);
    ...
    const handleSubmit = async (e) => {
        ...
        const menuData = {
            date: formData.date,
            morning: menuItems,
            afternoon: [], // Add empty afternoon array
            evening: []    // Add empty evening array
        };

        try {
            const response = await fetch(`http://localhost:5000/api/menu/date/${id}`, {
                method: "PUT",
                headers: {
            ...
            if (response.ok) {
                ...
                toast.error(errorData.error || "Failed to update menu");
            }
        } catch (error) {
        ...
        <>
            ...
            <div className="container mt-5" style={{ background: "white", color: "black", padding: "20px", borderRadius: "10px" }}>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <button type="submit" className="btn btn-dark">Update Menu</button>
                </form>
            </div>
        </>
    );

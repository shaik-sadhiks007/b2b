...
const Expiry = () => {
  ...
  }, [menuItems, monthsFilter]);

  const refreshExpiringItems = () => {
    setLoading(true);
    try {
  ...
  const sortedItems = React.useMemo(() => {
    let sortableItems = [...expiringItems];
    if (sortConfig.key) {
  ...
  const handleMonthsFilterChange = (e) => {
    e.preventDefault();
    setExpiryFilter(monthsFilter * 30); // Convert months to days for the context
    refreshExpiringItems();
  };

  const getSeverity = (months) => {
    if (months <= 1) return 'critical';  // 1 month or less
    if (months <= 3) return 'warning';   // 1-3 months
  ...
  const exportToCSV = () => {
    const headers = ['Name', 'Category', 'Subcategory', 'Expiry Date', 'Months Remaining', 'Days Remaining', 'Quantity'];
    const csvContent = [
      headers.join(','),
            ...
            <div className="bg-white rounded-lg shadow mb-6">
              <div className="p-4 md:p-6 pb-0 flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
                <div className="flex items-center gap-1">
                  ...
                  <h1 className="text-xl font-bold">Expiring Soon Items</h1>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button onClick={refreshExpiringItems} className="flex items-center gap-1 px-3 py-2 text-sm font-medium border rounded-md hover:bg-gray-50">
              ...
              <div className="p-4 md:p-6 pt-4 grid grid-cols-1 md:grid-cols-4 gap-2">
                ...
                <form onSubmit={handleMonthsFilterChange} className="flex gap-1">
                  <input
                    type="number"
                    min="1"
                ...
                <div className="p-12 text-center">
                  ...
                  <p className="mt-2 text-gray-500">
                    {searchTerm || filterCategory !== 'all'
                      ? 'No matching expiring items found'
                      : `No items expiring within ${monthsFilter} months`}
                  </p>
                </div>
              </div>
            ) : (
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th onClick={() => handleSort('formattedExpiry')} className="px-2 py-1 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                          <div className="flex items-center gap-1">
                            Expiry Date
                            {sortConfig.key === 'formattedExpiry' && (sortConfig.direction === 'asc' ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />)}
                          </div>
                        </th>
                        <th onClick={() => handleSort('monthsToExpiry')} className="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                          <div className="flex items-center justify-end gap-1">
                            Months Remaining
                            {sortConfig.key === 'monthsToExpiry' && (sortConfig.direction === 'asc' ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />)}
                          </div>
                        </th>
                        <th onClick={() => handleSort('daysToExpiry')} className="px-2 py-1 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                          <div className="flex items-center justify-end gap-1">
                            Days Remaining
                            {sortConfig.key === 'daysToExpiry' && (sortConfig.direction === 'asc' ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />)}
                          </div>
                        </th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {filteredItems.map((item) => {
                        const severity = getSeverity(item.monthsToExpiry);
                        return (
                          <tr key={item._id} className="hover:bg-gray-50">
                            ...
                            <td className="px-2 py-1 whitespace-nowrap text-sm text-gray-500">{item.formattedExpiry}</td>
                            <td className="px-2 py-1 whitespace-nowrap text-right">
                              <span className={`px-2 py-1 text-xs font-medium rounded-full border ${severityColors[severity]}`}>
                                {item.monthsToExpiry} {item.monthsToExpiry === 1 ? 'month' : 'months'}
                              </span>
                            </td>
                            <td className="px-2 py-1 whitespace-nowrap text-right text-sm text-gray-500">
